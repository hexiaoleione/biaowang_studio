...
import com.hex.express.iwant.minfragment.MinFragment1.SendEscortAdapter.OwnerViewHolder;
...
public class MinFragment2 extends Fragment {
	...
	public void getData() {
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int position, long arg3) {
				...
				intent.putExtra("recId", String.valueOf(data.recId));// int
																		// recId;//镖件主键
				intent.putExtra("toUserId", data.userId);// String
															// publishTime;//发布时间
				intent.putExtra("publishTime", data.publishTime);// String
																	// publishTime;//发布时间
				intent.putExtra("personName", data.personName);// String
																// personName;//发件人
				intent.putExtra("mobile", data.mobile);// String mobile;//发件人手机号
				intent.putExtra("address", data.address.replace("中国", ""));// String
																			// address;//发件地址
				intent.putExtra("personNameTo", data.personNameTo);// String
																	// personNameTo;//收件人
				intent.putExtra("matWeight", data.matWeight);// String
																// personNameTo;//重量
				intent.putExtra("mobileTo", data.mobileTo);// String
															// mobileTo;//收件人手机号
				intent.putExtra("addressTo", data.addressTo.replace("中国", ""));// String
																				// addressTo;//收件地址
				intent.putExtra("matRemark", data.matRemark);// String
																// matRemark;//物品备注
				intent.putExtra("matName", data.matName);// String
															// matRemark;//物品名字
				intent.putExtra("transferMoney", String.valueOf(data.transferMoney));// String
																						// matRemark;//物品价格
				...
				intent.putExtra("distance", data.distance);// 距离
				...
				if (!data.length.equals("0") && !data.wide.equals("0") && !data.high.equals("0")) {
					intent.putExtra("length",
							"长：" + data.length + " 厘米" + "  宽：" + data.wide + " 厘米" + "  高：" + data.high + " 厘米");
				...
				// intent.putExtra("length", "长："+data.length+" 宽："+data.wide+"
				// 高："+data.high);//物品体积
				intent.putExtra("status", data.status);// 状态 0-预发布成功(未支付)
														// 1-支付成功(已支付) 2(已抢单) 3
														// 已取件 4 订单取消()镖师 5 成功 6
														// 删除 7 已评价 8订单取消（用户）
	...
	class SendEscortAdapter extends BaseListAdapter {
		...
		public int getLayoutResource() {
			// TODO Auto-generated method stub
			return R.layout.item_newsendfreight;// item_sendfreight
												// item_newsendfreight
		...
		class OwnerViewHolder extends ViewHolder {
			...
			public void setData(final int position) {
				...
				if (bean.data.get(position).ifReplaceMoney.equals("true")
						&& !bean.data.get(position).ifReplaceMoney.equals("")) {
				...
				// /状态 0-预发布成功(未支付) 1-支付成功(已支付) 2(已抢单) 3 已取件(不需要代收款) 4 订单取消(镖师)
				// 5 成功 6 系统删除 7 已评价 8订单取消(用户)
				...
				if (bean.data.get(position).status.equals("0") || bean.data.get(position).status.equals("1")
					...
					text_quxiao.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View arg0) {
							...
							ad.setTitle("温馨提示");
							ad.setMessage("确认取消订单吗? 取消订单将会罚款");
						...
						private void deleteOrder() {
							AsyncHttpUtils.doSimGet(UrlMap.getTwo(MCUrl.DiverCance, "recId",
									"" + bean.data.get(position).recId, "type", "1"), new AsyncHttpResponseHandler() {
										...
										public void onSuccess(int arg0, Header[] arg1, byte[] arg2) {
											...
											// BaseBean baseBean= new
											// Gson().fromJson(new String(arg2),
											// BaseBean.class);
											...
											if (bean.getErrCode() == 0) {// 如果取消订单成功
												...
												getHttpRequst(false, true, 1, false);
												// finish();//
												// 取消订单成功后即销毁此页面,并去启动上一个页面的Fragment
												// startActivity(new
				...
				if (!bean.data.get(position).limitTime.equals("")) {
					// new MyBitmapUtils().display(iv_imgOfGood,
					// bean.data.get(position).matImageUrl);
				...
				try {
					...
					Date d1 = df.parse(bean.data.get(position).useTime);
