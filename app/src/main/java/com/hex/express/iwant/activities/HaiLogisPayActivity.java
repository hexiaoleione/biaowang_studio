...
import com.hex.express.iwant.activities.HaiLogisPayActivity.MyReceiver;
 ...
 * 发物流支付界面
 */
public class HaiLogisPayActivity extends BaseActivity {
	...
	@Bind(R.id.pay_surpluse)//支付按钮
	ImageView  pay_surpluse;
//	@Bind(R.id.pay_wenxine)//支付按钮
//	ImageView  pay_wenxin;
//	@Bind(R.id.pay_zhifubaoe)//支付按钮
//	ImageView  pay_zhifubao;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_hailogispaynew);//activity_logispay activity_logispaynew
		ButterKnife.bind(HaiLogisPayActivity.this);
...
//		getDiscount();
		logispay_money.setText(getIntent().getStringExtra("insureCost"));
		logispay_moneys.setText(getIntent().getStringExtra("insureCost"));
...
//	
		...
		pay_surpluse.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View arg0) {
				// TODO Auto-generated method stub
				money=getIntent().getStringExtra("insureCost");
//				logispay_money.setText(getIntent().getStringExtra("insureCost")+"元");
				logispay_moneys.setText(getIntent().getStringExtra("insureCost")+"元");
//				pay_surplus.setBackgroundResource(R.drawable.underclick_03);
//				pay_surpluse.setBackgroundResource(R.drawable.underclick_03);
...
//				ways=1;
				getaddsurplus();
				Log.e("1111111msg", "WLBId"+""+getIntent().getIntExtra("recId", 0)+"transferMoney"+ money);
			}
		});
	}
	...
	@OnClick({ R.id.lout_showdown, R.id.lout_show, R.id.pay_surpluse, R.id.pay_zhifubaoe, R.id.pay_wenxine ,R.id.logis_paybtn})
...
//			getAllpay();
//			pay_zhifubao.setClickable(false);
//			logispay_money.setText(getIntent().getStringExtra("insureCost")+"元");
//			logispay_moneys.setText(getIntent().getStringExtra("insureCost")+"元");
//			pay_wenxin.setBackgroundResource(R.drawable.weixin_noclick_05);
...
//				new AsyncHttpResponseHandler() {
		AsyncHttpUtils.doGet(UrlMap.getThreeUrl(MCUrl.WPRE, "billCode",billCode,
	            ...
	            if (bean.getErrCode()==0) {
					...
					ToastUtil.shortToast(HaiLogisPayActivity.this, bean.getMessage());
				}
					}
					@Override
...
//				Toast.makeText(LogisPayActivity.this, "支付成功", Toast.LENGTH_SHORT).show();
				...
				if (TextUtils.equals(resultStatus, "8000")) {
					Toast.makeText(HaiLogisPayActivity.this, "支付结果确认中", Toast.LENGTH_SHORT).show();

				} else {
					// 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
					Toast.makeText(HaiLogisPayActivity.this, "支付失败", Toast.LENGTH_SHORT).show();
...
//		Log.e("obj", UrlMap.getThreeUrl(MCUrl.Balance, "userId", String
...
//		new AsyncHttpResponseHandler() {
		Log.e("1111111msg", "WLBId"+""+getIntent().getIntExtra("recId", 0)+"transferMoney"+ money);
		AsyncHttpUtils.doGet(UrlMap.getfive(MCUrl.Balance, "userId",""+ String
				.valueOf(PreferencesUtils.getInt(getApplicationContext(),
						PreferenceConstants.UID)),"WLBId",""+getIntent().getIntExtra("recId", 0), 
				"warrant",""+1,"whether",""+1,"transferMoney", money), null, null, params,
				new AsyncHttpResponseHandler() {
					@Override
					public void onSuccess(int arg0, Header[] arg1, byte[] arg2) {
						...
						if (arg2 == null)
							return;
						Log.e("11111msg", new String(arg2));
						BaseBean bean = new Gson().fromJson(new String(arg2), BaseBean.class);
						if (bean.getErrCode() == 0) {
							ToastUtil.shortToast(HaiLogisPayActivity.this, "支付成功");
//							sendBroadcast(new Intent().setAction("freight").putExtra("flag", "owner"));//启动SendFreightActivity界面	
//							intent.setClass(HaiLogisPayActivity.this, MylogisticakActivity.class);//个人
//							setResult(RESULT_OK, intent);
//							startActivity(intent);
							...
							ToastUtil.shortToast(HaiLogisPayActivity.this, bean.getMessage());
							finish();
//							DialogUtils.createAlertDialogTwo(HaiLogisPayActivity.this, "", bean.getMessage(), 0, "去充值",
//									"更换支付方式", new DialogInterface.OnClickListener() {
//
//										@Override
//										public void onClick(DialogInterface dialog, int which) {
//
//											startActivity(new Intent(HaiLogisPayActivity.this, RechargeActivity.class));
//										}
//
//									}).show();
						}
					...
					public void onFailure(int arg0, Header[] arg1, byte[] arg2, Throwable arg3) {
						// TODO Auto-generated method
						// stub
						ToastUtil.shortToast(getApplicationContext(), "请求失败");
						dialog.dismiss();
					}
				});
	}
				...
				new AsyncHttpResponseHandler() {
					...
					public void onSuccess(int arg0, Header[] arg1, byte[] arg2) {
						...
						if (bean.getErrCode() == 0) {
							ToastUtil.shortToast(HaiLogisPayActivity.this, bean.getMessage());
//							sendBroadcast(new Intent().setAction("freight").putExtra("flag", "owner"));//启动SendFreightActivity界面	
							intent.setClass(HaiLogisPayActivity.this, LogistcaInforseActivity.class);//公司
							startActivity(intent);
							finish();
						}
